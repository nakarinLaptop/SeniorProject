...
<%
		...
		try {
			...
			for (int i = days - 1; i >= 0; i--) {
				...
				stmt = con.prepareStatement(
					"SELECT COUNT(*) FROM prediction WHERE DATE(created_at) = ? AND predict_result = 'home'");
				...
				stmt = con.prepareStatement(
					"SELECT COUNT(*) FROM prediction WHERE DATE(created_at) = ? AND predict_result = 'hospital'");
				stmt.setString(1, dateStr);
				rs = stmt.executeQuery();
	...
	if (dateParam != null) {
		...
		try {
			stmt = con.prepareStatement("SELECT COUNT(*) FROM prediction WHERE DATE(created_at) = ?");
			stmt.setString(1, dateParam);
			rs = stmt.executeQuery();
			if (rs.next()) totalPredict = rs.getInt(1);
			rs.close(); stmt.close();

			stmt = con.prepareStatement("SELECT COUNT(*) FROM prediction WHERE DATE(created_at) = ? AND predict_result = 'home'");
			stmt.setString(1, dateParam);
			rs = stmt.executeQuery();
			if (rs.next()) homeCount = rs.getInt(1);
			rs.close(); stmt.close();

			stmt = con.prepareStatement("SELECT COUNT(*) FROM prediction WHERE DATE(created_at) = ? AND predict_result = 'hospital'");
			stmt.setString(1, dateParam);
			rs = stmt.executeQuery();
			if (rs.next()) hospitalCount = rs.getInt(1);
			rs.close(); stmt.close();

			stmt = con.prepareStatement("SELECT COUNT(*) FROM prediction WHERE DATE(created_at) = ? AND predict_result = doctor_selected");
			stmt.setString(1, dateParam);
			rs = stmt.executeQuery();
	...
	try {
		...
		stmt = con.prepareStatement("SELECT COUNT(*) FROM user WHERE role = 'doctor'");
		rs = stmt.executeQuery();
		if (rs.next()) doctorCount = rs.getInt(1);
		rs.close(); stmt.close();

		stmt = con.prepareStatement("SELECT COUNT(*) FROM patient");
		rs = stmt.executeQuery();
		if (rs.next()) patientCount = rs.getInt(1);
		rs.close(); stmt.close();

		stmt = con.prepareStatement("SELECT COUNT(*) FROM user WHERE role = 'staff'");
		rs = stmt.executeQuery();
		if (rs.next()) staffCount = rs.getInt(1);
		rs.close(); stmt.close();

		stmt = con.prepareStatement("SELECT COUNT(*) FROM prediction WHERE DATE(created_at) = CURDATE()");
		rs = stmt.executeQuery();
		if (rs.next()) totalPredict = rs.getInt(1);
		rs.close(); stmt.close();

		stmt = con.prepareStatement("SELECT COUNT(*) FROM prediction WHERE DATE(created_at) = CURDATE() AND predict_result = 'home'");
		rs = stmt.executeQuery();
		if (rs.next()) homeCount = rs.getInt(1);
		rs.close(); stmt.close();

		stmt = con.prepareStatement("SELECT COUNT(*) FROM prediction WHERE DATE(created_at) = CURDATE() AND predict_result = 'hospital'");
		rs = stmt.executeQuery();
		if (rs.next()) hospitalCount = rs.getInt(1);
		rs.close(); stmt.close();

		stmt = con.prepareStatement("SELECT COUNT(*) FROM prediction WHERE DATE(created_at) = CURDATE() AND predict_result = doctor_selected");
		rs = stmt.executeQuery();
		if (rs.next()) agreeCount = rs.getInt(1);
		rs.close(); stmt.close();
...
<body>
	<%
	request.setAttribute("activePage", "adminHome");
	request.setAttribute("pageTitle", "หน้าแรก");
	%>
	...
	<div class="content" id="dashboardPage">
		...
		<div class="system-stat-row">
			<div class="system-stat-card">
				<p class="big-num" style="color: #007bff">หมอภายในระบบ <%= doctorCount %> คน</p>
			</div>
			<div class="system-stat-card">
				<p class="big-num" style="color: #28a745">คนไข้ภายในระบบ <%= patientCount %> คน</p>
			</div>
			<div class="system-stat-card">
				<p class="big-num" style="color: #ffc107">เจ้าหน้าที่ตรวจภายในระบบ <%= staffCount %> คน</p>
			</div>
		</div>
